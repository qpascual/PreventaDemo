...
Ext.define('Preventa.view.CatalegProductesPortrait', {
    extend: 'Ext.Container',
    xtype: 'CatalegProductesPortrait',
    ...
    createCarousel: function(category) {
   	...
   	//Afegim el carousel de imatges.
    	...
    	store.filter([{property: "id_category", value: category['id'], exactMatch: true}]);
    	...
    	var totalPagines = Math.ceil(store.getCount()/itemsPerPagina);
	    	...
	    	for(var i = 1; i <= totalPagines; i++){
	    		...
	    		while(espaiWidth <= 25 || espaiHeight <= 25){
	    			...
	    			espaiHeight = (carouselHeight - (imageSize * 4)) / 4;
		    	}
	...
	isCreated: function(){
		console.log("cataleg portrait iscreated with: " + this.getItems()['items'].length);
		return this.getItems()['items'].length >= 2;
	},
    ...
    onChooseClient: function(){
    	this.fireEvent('onChooseClient', 'CatalegProductes');
    },
